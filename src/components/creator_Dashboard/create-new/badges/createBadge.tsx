  ...
  const handleIconUpload = (e) => {
    ...
    if (file) {
      ...
      if (!file.type.startsWith("image/")) {
        setNotification({
          type: 'error',
          message: 'Please select an image file (JPG, PNG, GIF, SVG)'
        })
        return
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!iconUrl) {
        setNotification({
          type: 'error',
          message: 'Please provide an icon (either upload a file or enter a URL)'
        })
        return
      }
      ...
      if (!formData.description.trim()) {
        setNotification({
          type: 'error',
          message: 'Please enter a badge description'
        })
        return
      }
      ...
      setNotification({
        type: 'success',
        message: `Badge "${result.name}" created successfully! Status: ${result.status}`
      })
      ...
      const errorMessage = err?.data?.message || err?.message || "Failed to create badge. Please try again."
      setNotification({
        type: 'error',
